...
def neg_log_likelihood(data, theta, beta):
    ...
    for i, j, is_correct in zip(data["user_id"], data["question_id"], data["is_correct"]):
        ...
        log_lklihood += is_correct * ((theta[i][j] - beta[j]) - np.log(1 + np.exp(theta[i][j] - beta[j])))
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
    return -log_lklihood
...
def update_theta_beta(data, lr, theta, beta, discriminant):
    ...
    for i, j, is_correct in zip(data["user_id"], data["question_id"], data["is_correct"]):
        p = sigmoid((theta[i][j] - beta[j]) * discriminant[j])  # Get probability of correct answer
        d_theta[i][j] += is_correct - p  # Get derivative w.r.t theta
...
def irt(data, val_data, test_data, lr, iterations, discriminant, labels: torch.Tensor, quiet=False):
    """ Train IRT model.
    ...
    theta = np.zeros((len(data["user_id"]), labels.size(0)))
...
def main():
    ...
    # cluster labels
    labels = question_cluster.cluster_labels()
    ...
    for lr in lr_list:
        _, _, val_acc_lst, test_acc_lst, _, val_lld_lst = irt(train_data, val_data, test_data, lr, max(iter_list),
                                                              discriminant, labels, quiet=True)
    ...
    with open("IRT_gridsearch_m.csv", "w", newline="") as f:
