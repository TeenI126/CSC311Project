...
def load_data(base_path="../data"):
    ...
    correct_train_matrix = train_matrix.copy()
    # Fill in the missing entries to 0.
    incorrect_train_matrix = train_matrix.copy()
    incorrect_train_matrix[train_matrix == 1] = 0
    incorrect_train_matrix[train_matrix == 0] = 1
    incorrect_train_matrix[np.isnan(train_matrix)] = 0

    # Changes wrongly answered questions to -1
    correct_train_matrix[np.isnan(train_matrix)] = 0
    # Change to Float Tensor for PyTorch.
    zero_train_matrix = np.concatenate((correct_train_matrix, incorrect_train_matrix), axis=1)
    zero_train_matrix = torch.FloatTensor(zero_train_matrix)
    train_matrix = torch.FloatTensor(np.concatenate((train_matrix, train_matrix == 0), axis=1))

    return zero_train_matrix, train_matrix, valid_data, test_data


class AutoEncoder(nn.Module):
    def __init__(self, num_question, k=100, j=100):
        ...
        # self.f = nn.Linear(j, num_question)
    ...
    def get_weight_norm(self):
        ...
        # f_w_norm = torch.norm(self.h.weight, 2) ** 2
...
def train(model, lr, lamb, train_data, zero_train_data, valid_data, num_epoch, plot_over_epoch=False):
    ...
    for epoch in range(0, num_epoch):
        ...
        if valid_acc < last_valid_acc:
            ...
            last_improvement = 0
        if last_improvement > 5 and epoch > 20:
            break
...
def evaluate(model, train_data, valid_data):
    ...
    for i, u in enumerate(valid_data["user_id"]):
        ...
        guess = output[0][valid_data["question_id"][i]].item() >= \
                output[0][valid_data["question_id"][i]+int(inputs.size(axis=1)/2)].item()
...
def main():
    ...
    for k in K:
        for lr in LR:
            print(f"\nk = {k}, lr = {lr}")
            model = AutoEncoder(train_matrix.size(1), k, k)
